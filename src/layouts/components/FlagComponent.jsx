...
export default function FlagComponent({ countryCode, size = 'l' }) {
    ...
    const checkCountryCode = () => {
        const checkedCountryCode = countryCodeConverter(countryCode);
        if (checkedCountryCode === 'NZL') {
            return <Box component={'img'} src={`/icons/flags/${size}/NZ.png`} sx={{ borderRadius: size === 's' ? '2px' : '4px' }} />;
        } else if (checkedCountryCode === 'KOS') {
            return <Box component={'img'} src={`/icons/flags/${size}/KO.png`} sx={{ borderRadius: size === 's' ? '2px' : '4px' }} />;
        } else if (countryList.find(country => country.alpha3 === checkedCountryCode)) {
            return <Flag code={checkedCountryCode} gradient='top-down' size={size} />;
        } else return <Box component='img' src='/img/img-placeholder_32.png' width={sizes(size)} sx={{ borderRadius: '4px' }} />;
    };
...
const countryCodeConverter = (code) => {
    if (code === 'ALG') return 'DZA';
    if (code === 'ASA') return 'ASM';
    if (code === 'ANG') return 'AGO';
    if (code === 'ANT') return 'ATG';
    if (code === 'ARU') return 'ABW';
    if (code === 'BAH') return 'BHS';
    if (code === 'BRN') return 'BHR';
    if (code === 'BAN') return 'BGD';
    if (code === 'BAR') return 'BRB';
    if (code === 'BIZ') return 'BLZ';
    if (code === 'BER') return 'BMU';
    if (code === 'BHU') return 'BTN';
    if (code === 'BOT') return 'BWA';
    if (code === 'IVB') return 'VGB';
    if (code === 'BRU') return 'BRN';
    if (code === 'BUL') return 'BGR';
    if (code === 'BUR') return 'BFA';
    if (code === 'CAM') return 'KHM';
    if (code === 'CAY') return 'CYM';
    if (code === 'CHA') return 'TCD';
    if (code === 'CHI') return 'CHL';
    if (code === 'CGO') return 'COG';
    if (code === 'CRC') return 'CRI';
    if (code === 'CRO') return 'HRV';
    if (code === 'DEN') return 'DNK';
    if (code === 'ESA') return 'SLV';
    if (code === 'GEQ') return 'GNQ';
    if (code === 'FIJ') return 'FJI';
    if (code === 'GAM') return 'GMB';
    if (code === 'GER') return 'DEU';
    if (code === 'GRE') return 'GRC';
    if (code === 'GRN') return 'GRD';
    if (code === 'GUA') return 'GTM';
    if (code === 'GUI') return 'GIN';
    if (code === 'GBS') return 'GNB';
    if (code === 'HAI') return 'HTI';
    if (code === 'HON') return 'HND';
    if (code === 'INA') return 'IDN';
    if (code === 'IRI') return 'IRN';
    if (code === 'KUW') return 'KWT';
    if (code === 'LAT') return 'LVA';
    if (code === 'LIB') return 'LBN';
    if (code === 'LES') return 'LSO';
    if (code === 'LBA') return 'LBY';
    if (code === 'MAD') return 'MDG';
    if (code === 'MAW') return 'MWI';
    if (code === 'MAS') return 'MYS';
    if (code === 'MTN') return 'MRT';
    if (code === 'MRI') return 'MUS';
    if (code === 'MON') return 'MCO';
    if (code === 'MGL') return 'MNG';
    if (code === 'MYA') return 'MMR';
    if (code === 'NEP') return 'NPL';
    if (code === 'NED') return 'NLD';
    if (code === 'NCA') return 'NIC';
    if (code === 'NIG') return 'NER';
    if (code === 'NGR') return 'NGA';
    if (code === 'OMA') return 'OMN';
    if (code === 'PLE') return 'PSE';
    if (code === 'PAR') return 'PRY';
    if (code === 'PHI') return 'PHL';
    if (code === 'POR') return 'PRT';
    if (code === 'PUR') return 'PRI';
    if (code === 'SKN') return 'KNA';
    if (code === 'VIN') return 'VCT';
    if (code === 'SAM') return 'WSM';
    if (code === 'KSA') return 'SAU';
    if (code === 'SEY') return 'SYC';
    if (code === 'SLO') return 'SVN';
    if (code === 'SOL') return 'SLB';
    if (code === 'RSA') return 'ZAF';
    if (code === 'SRI') return 'LKA';
    if (code === 'SUD') return 'SDN';
    if (code === 'SUI') return 'CHE';
    if (code === 'TPE') return 'TWN';
    if (code === 'TAN') return 'TZA';
    if (code === 'TOG') return 'TGO';
    if (code === 'TGA') return 'TON';
    if (code === 'UAE') return 'ARE';
    if (code === 'ISV') return 'VIR';
    if (code === 'URU') return 'URY';
    if (code === 'VAN') return 'VUT';
    if (code === 'VIE') return 'VNM';
    if (code === 'ZAM') return 'ZMB';
    if (code === 'ZIM') return 'ZWE';
    else return code;
};
...
const countryList = [
    ...
    { countryName: "United Kingdom", alpha2: "GB-UKM", alpha3: "GBR", numeric: "836" },
    ...
    { countryName: "Saint Helena, Ascension Island, Traistan da Cunha", alpha2: "SH", alpha3: "SHN", numeric: "654" },
